public class BinaryTree<V extends Comparable<V>> {
    ...
    public void flattenHelper(Node<V> node, Comparable[] a){
        if (node == null){
            return;
        }
        a[c] = node.getValue();
        c++;
        flattenHelper(node.getLeft(), a);
        flattenHelper(node.getRight(),a);
    }
    // bubble sort
    // useful for flatten
    ...
    public Node<V> invertHelper(Node<V> node) {
        ...
        if (node.getRight() != null){
            Node<V> temp = new Node(node.getLeft().getValue(), node.getLeft().getLeft(), node.getLeft().getRight());
